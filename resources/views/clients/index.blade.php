...
@section('content')
    <div class="mb-3">
        ...
        <a href="{{ route('clients.create') }}" class="btn btn-primary btn-sm" style="margin-left: 10px;"> + Register a new Client </a>
        @if ($errors->any())
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($data as $v)
                                <tr>
                                    <td>
                                        <a href="{{ route('clients.edit', ['client' => $v->id]) }}" class="btn btn-warning btn-sm">Edit</a>
...
<script>
    $('#select-page').change(function(e){
        if (this.value != null && this.value != "") {
            ...
            window.location='{{ route('clients.index') }}?'+searchParams.toString();
        }
    });
